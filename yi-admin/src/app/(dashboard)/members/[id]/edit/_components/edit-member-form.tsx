...
export function EditMemberForm({ member, verticals }: {
  ...
  const [form, setForm] = useState({
    ...
    industry: INDUSTRIES.includes(member.industry ?? '') ? (member.industry ?? '') : (member.industry ? 'Other' : ''),
    industry_other: INDUSTRIES.includes(member.industry ?? '') ? (member.industry_other ?? '') : (member.industry ?? ''),
