...
export default async function ViewMemberPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: member } = await adminClient.from('profiles').select('*').eq('id', id).single()
  if (!member) notFound()
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {member.member_type.replace('_', ' ')} · Joined {format(new Date(member.created_at), 'd MMM yyyy')}
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {hasPersonal && (
            <Section title="Personal">
              <InfoRow label="Date of Birth" value={member.dob ? format(new Date(member.dob), 'd MMM yyyy') : null} />
              ...
              <InfoRow label="Anniversary" value={member.anniversary_date ? format(new Date(member.anniversary_date), 'd MMM yyyy') : null} />
              {member.personal_bio && (
                <div className="pt-2">
          ...
          <Section title="Account">
            ...
            <InfoRow label="Joined" value={format(new Date(member.created_at), 'd MMM yyyy')} />
